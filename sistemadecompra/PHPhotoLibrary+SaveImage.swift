...
//  sistemadecompra
//
//  Created by Richard Gomez on 2/23/18.
    ...
    fileprivate func saveImage(image: UIImage, album: PHAssetCollection, completion:((PHAsset?)->())? = nil) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if success {
                ...
                let assets:PHFetchResult<PHAsset> =  PHAsset.fetchAssets(withLocalIdentifiers: [placeholder.localIdentifier], options: fetchOptions)
                let asset:PHAsset? = assets.firstObject
                print(asset?.value(forKey: "filename"))
                print(asset?.value(forKey: "creationDate"))
                completion?(asset)
            } else {
                completion?(nil)
            }
